...
from app.schemas.metrica_estudio import MetricaEstudioResponse, TiempoEstudioUpdate
...
@router.post("/me/tiempo-estudio", response_model=MetricaEstudioResponse)
def sumar_tiempo_estudio(
    payload: TiempoEstudioUpdate,
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
    return metrica_estudio_service.acumular_tiempo_estudio(
        db, current_user.id, payload.segundos_a_agregar
    )